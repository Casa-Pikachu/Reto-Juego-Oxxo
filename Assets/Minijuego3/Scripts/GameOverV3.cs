...
public class GameOverV3 : MonoBehaviour
{
    ...
    void ActualizarDatosEnServidor(int puntos, int experiencia, int monedas)
    {
        ...
        StartCoroutine(EnviarPUT("https://10.22.179.245:7222/Usuarios/UpdatePuntos", $"{idUsuario}/{nuevosPuntos}"));
        StartCoroutine(EnviarPUT("https://10.22.179.245:7222/Usuarios/UpdateMonedas", $"{idUsuario}/{nuevasMonedas}"));
        StartCoroutine(EnviarPUT("https://10.22.179.245:7222/Usuarios/UpdateExperiencia", $"{idUsuario}/{nuevaExperiencia}"));
